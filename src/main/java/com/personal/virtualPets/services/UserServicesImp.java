		...
		if (userRepo.existsById(id)) {
			...
			if (!data.getPassword().isEmpty() && user.getPassword()==data.getPassword()) {
				if (!data.getUsername().isEmpty()) {
					if (userRepo.existsByUsername(data.getUsername()) && !user.equals(userRepo.findByUsername(data.getUsername())))
			...
			else return new ResponseEntity<> ("Change not allowed", HttpStatus.BAD_REQUEST);
		}
		return new ResponseEntity<> ("User not found", HttpStatus.NO_CONTENT);
	}

	//TODO verification creates a profile
	@Override
